...
export type WithdrawalValidationErrors = {
  name?: string;
  amount?: string;
  date?: string;
};
...
export async function listWithdrawalsByMonth(month: string) {
  const supabase = getSupabaseBrowserClient();
...
export async function createWithdrawal(input: WithdrawalInput) {
  ...
  const payload: WithdrawalInsert = {
    user_id: userId,
...
export async function updateWithdrawal(withdrawalId: string, input: WithdrawalInput) {
  const supabase = getSupabaseBrowserClient();
  const userId = await getAuthenticatedUserId();
